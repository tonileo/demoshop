...
export class StripeService {
  ...
  async createPaymentElement(){
    if (!this.paymentElement) {
      const elements = await this.initialiteElements();
      if (elements) {
        this.paymentElement = elements.create('payment');
      }else{
        throw new Error("Elements instace has  not been initialized");
      }
    }
  ...
  async createConfirmationToken () {
    ...
    if (stripe){
      ...
      throw new Error('Stripe not availiable');
    }
  }
